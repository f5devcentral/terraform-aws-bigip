...
resource "aws_instance" "f5_bigip" {
  ...
  provisioner "local-exec" {
    command = "ssh -i ${var.private_key_path} -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o ConnectionAttempts=20 -v admin@${self.public_dns} 'modify auth user admin password \"${random_string.password.result}\"'"
  }
  ...
  #
  provisioner "local-exec" {
    command = "ssh -i ${var.private_key_path} -o StrictHostKeyChecking=no -o ConnectTimeout=10 -o ConnectionAttempts=10 -v admin@${self.public_dns} 'modify auth user admin shell bash'"
  }
